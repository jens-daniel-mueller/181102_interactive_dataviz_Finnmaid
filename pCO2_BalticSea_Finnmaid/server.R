...
server <- function(input, output) {
  ...
  output$ValuesPerPoint <- renderText({
    if(input$routeall == TRUE)
      Sub <<- df %>% 
        dyplr::filter(date >=input$daterange[1] & date <=input$daterange[2] & 
    ...
    if (input$routeE == TRUE)
      Sub <<- df %>% 
        dyplr::filter(route=="E" & date >=input$daterange[1] & date <=input$daterange[2] & 
                 Lon >= input$lon_low & Lon <= input$lon_high &
                 Lat >=input$lat_low & Lat <= input$lat_high ) %>% 
        dplyr::select(date,Lon,Lat,pCO2,Tem,ID) else {NULL} 
    if (input$routeW == TRUE)
      Sub <<-rbind(Sub,df %>% 
                     dyplr::filter(route == "W", date >=input$daterange[1] & date <=input$daterange[2] & 
                              Lon >= input$lon_low & Lon <= input$lon_high &
                              Lat >=input$lat_low & Lat <= input$lat_high ) %>% 
                     dplyr::select(date,Lon,Lat,pCO2,Tem,ID)) else {NULL} 
    ...
    if(input$routeP == TRUE)
      Sub <<-rbind(Sub,df %>% 
                     dyplr::filter(route == "P", date >=input$daterange[1] & date <=input$daterange[2] & 
                              Lon >= input$lon_low & Lon <= input$lon_high &
                              Lat >=input$lat_low & Lat <= input$lat_high ) %>% 
                     dplyr::select(date,Lon,Lat,pCO2,Tem,ID)) else {NULL}
    if(input$routeG == TRUE)
      Sub <<-rbind(Sub,df %>% 
                     dyplr::filter(route == "G", date >=input$daterange[1] & date <=input$daterange[2] & 
                              Lon >= input$lon_low & Lon <= input$lon_high &
                              Lat >=input$lat_low & Lat <= input$lat_high ) %>% 
                     dplyr::select(date,Lon,Lat,pCO2,Tem,ID)) else {NULL}
  ...
  output$plot_checkbox <- renderPlotly({
    ...
    if(input$routeS == TRUE)
      Sub_pCO2 <<-rbind(Sub_pCO2,df %>% 
                          dyplr::filter(route == "S", date >=input$daterange[1] & date <=input$daterange[2] & 
                                   Lon >= input$lon_low & Lon <= input$lon_high &
                                   Lat >=input$lat_low & Lat <= input$lat_high ) %>% 
                          dplyr::select(date,Lon,Lat,pCO2,Tem,ID, Sal, cO2)) else {NULL} 
    if(input$routeP == TRUE)
  ...
  output$trans_pCO2 <- renderPlotly({
    ...
    if(input$routeall == TRUE)
      trans_sub <<- df %>% 
        dyplr::filter(date >=input$daterange[1] & date <=(input$daterange[1]+30) & 
                 Lon >= input$lon_low & Lon <= input$lon_high &
                 Lat >=input$lat_low & Lat <= input$lat_high ) %>% 
        dplyr::select(date,Lon,pCO2, Lat, Sal, Tem, cO2) else {NULL} 
    if (input$routeE == TRUE)
      trans_sub <- df %>% 
        dyplr::filter(route == "E", date >=input$daterange[1] & date <=(input$daterange[1]+30) & 
                 Lon >= input$lon_low & Lon <= input$lon_high &
                 Lat >=input$lat_low & Lat <= input$lat_high ) %>% 
        dplyr::select(date,Lon,pCO2, Lat, Sal, Tem, cO2) else {NULL} 
    if (input$routeW == TRUE)
      trans_sub <-rbind(trans_sub,df %>% 
                          dyplr::filter(route == "W", date >=input$daterange[1] & date <=(input$daterange[1]+30) & 
                                   Lon >= input$lon_low & Lon <= input$lon_high &
                                   Lat >=input$lat_low & Lat <= input$lat_high ) %>% 
    ...
    if(input$routeP == TRUE)
      trans_sub <-rbind(trans_sub,df %>% 
                          dyplr::filter(route == "P", date >=input$daterange[1] & date <=(input$daterange[1]+30) & 
                                   Lon >= input$lon_low & Lon <= input$lon_high &
                                   Lat >=input$lat_low & Lat <= input$lat_high ) %>% 
                          dplyr::select(date,Lon,Lat,pCO2, Sal, Tem, cO2)) else {NULL}
    if(input$routeG == TRUE)
      trans_sub <-rbind(trans_sub,df %>% 
                          dyplr::filter(route == "G", date >=input$daterange[1] & date <=(input$daterange[1]+30) & 
                                   Lon >= input$lon_low & Lon <= input$lon_high &
                                   Lat >=input$lat_low & Lat <= input$lat_high ) %>% 
                          dplyr::select(date,Lon,Lat,pCO2, Sal, Tem, cO2)) else {NULL}
  ...
  output$trans_temp <- renderPlotly({
    ...
    if(input$routeS == TRUE)
      trans_sub <-rbind(trans_sub,df %>% 
                          dyplr::filter(route == "S", date >=input$daterange[1] & date <=(input$daterange[1]+30) & 
                                   Lon >= input$lon_low & Lon <= input$lon_high &
                                   Lat >=input$lat_low & Lat <= input$lat_high ) %>% 
                          dplyr::select(date,Lon,Lat,pCO2, Sal, Tem, cO2)) else {NULL} 
    if(input$routeP == TRUE)
